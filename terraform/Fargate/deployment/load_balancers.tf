...
resource "aws_alb" "application_load_balancer" {
  name               = "test-lb-tf-go" # Naming our load balancer
...
resource "aws_lb_target_group" "target_group" {
  name        = "apm-tutorial-target-group-go"
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
...
resource "aws_alb" "application_load_balancer_2" {
  name               = "test-lb-tf-2-go" # Naming our load balancer
  load_balancer_type = "application"
...
resource "aws_lb_target_group" "target_group_2" {
  name        = "apm-tutorial-target-group-2-go"
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = "${aws_default_vpc.default_vpc.id}" # Referencing the default VPC
